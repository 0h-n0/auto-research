...
| **`research.compute.shop` で gpu_type が catalog 未収録** (v0.8.0+) | aliases lookup 失敗 | fuzzy 候補を 3 つ表示、`gpu_providers.json` への PR 案内 |
| **全 provider が予算超過** (v0.8.0+) | `max_usd_per_hour` でフィルタ後 0 件 | top 5 を warning 付きで表示 + smaller GPU / spot / academic 提案 |
| **catalog が古い (90 日超)** (v0.8.0+) | `gpu_providers.json.updated_at` 比較 | 90 日: note 出力、180 日: warning、365 日: test fail (CI 検出) |
| **`cost_overrides.json` 不在で実費試算が公開価格ベース** (v0.8.0+) | research.cost.estimate 出力 | catalog の `pricing_source` を明示、契約価格は override 推奨 (git ignore) |
